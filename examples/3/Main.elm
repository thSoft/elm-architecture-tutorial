...
rootUrl : String
rootUrl =
  "https://thsoft.firebaseio.com/elm-architecture-tutorial/3"
...
view address model =
  let result =
        Html.div [] ([insert, remove] ++ counterViews)
      insert =
        button [ onClick address Create ] [ text "Add" ]
      remove =
        let result =
              button [ onClick address action ] [ text "Remove" ]
            action =
              model.urls |> List.head |> Maybe.map Remove |> Maybe.withDefault NoAction
        in result
      counterViews =
